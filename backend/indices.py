...
# Base acquisition constraints. These are the *preferred* values — if no
# scene satisfies them, _select_scene() progressively relaxes the cloud
# threshold and widens the search window (see _SEARCH_FALLBACKS) instead
# of failing outright. A single fixed window/threshold combination would
# return zero scenes (and therefore null results in the UI) for any AOI
# going through a long cloudy spell, e.g. monsoon season over Dehradun.
MAX_CLOUD_PERCENT  = 5
SEARCH_WINDOW_DAYS = 90

# Progressive fallback steps tried in order until at least one scene is
# found: (search_window_days, max_cloud_percent).
_SEARCH_FALLBACKS = [
    (SEARCH_WINDOW_DAYS, MAX_CLOUD_PERCENT),
    (180, 20),
    (365, 40),
    (365, 80),
]
...
def _scenes_for_window(geometry: "ee.Geometry", window_days: int, cloud_percent: float):
    """
    Builds the cloud-masked, newest-first S2 collection for a given
    (window_days, cloud_percent) combination, joined against
    s2cloudless. Returns (collection, image_count, start_str, end_str).
    """
    ...
    return collection, image_count, start_str, end_str


def _select_scene(geometry: "ee.Geometry", label: str = ""):
    """
    Shared scene-selection logic used by both calculate() and
    cross_section(). Picks the newest Sentinel-2 scene where most of
    `geometry` is actually unmasked after cloud masking.

    Tries _SEARCH_FALLBACKS in order — starting with the strict
    preferred (SEARCH_WINDOW_DAYS, MAX_CLOUD_PERCENT) pass and
    progressively widening the date range / relaxing the cloud
    threshold — so a long cloudy spell over the AOI degrades to a
    best-effort (but non-null) result instead of failing outright.

    Returns (image, scene_date, image_count). Raises ValueError only if
    no scene is found even at the widest/most permissive fallback.
    """
    collection = image_count = start_str = end_str = None
    window_days = cloud_percent = None

    for window_days, cloud_percent in _SEARCH_FALLBACKS:
        collection, image_count, start_str, end_str = _scenes_for_window(
            geometry, window_days, cloud_percent
        )
        logger.info(
            f"[{label}] {image_count} scene(s) found  "
            f"| {start_str} → {end_str}  | cloud < {cloud_percent}%"
        )
        if image_count > 0:
            break

    if image_count == 0:
        widest_days, widest_cloud = _SEARCH_FALLBACKS[-1]
        raise ValueError(
            f"No Sentinel-2 imagery found even within the last {widest_days} days "
            f"with less than {widest_cloud}% cloud cover."
        )
...
def calculate(
    ...
    scene available (see _select_scene for the fallback search policy).
